...
<html lang="en">
  ...
  <body>
      ...
      <div class="col-sm-6 col-md-4 col-lg-3">
        <div class="panel panel-default panel-pet">
          ...
          <div class="panel-body">
            ...
            <button  class="btn btn-alert btn-adopt" type="button" data-id="0">Adopt</button>
          </div>
        </div>
      </div>
    </div>
   ...
   function petAdopt(){
	   //버튼 클릭-> 분양 테이블 등록
	   $('.panel-body').on("click","btn btn-alert btn-adopt",function(){
   ...
   function perList(){
   		...
   		$.ajax({
   			...
   			for(pet of list){
   				...
   				/* $('<div>').attr('class','col-sm-6 col-md-4 col-lg-3').appendTo('#petTemplate');
   				$('<div>').attr('class','panel panel-default panel-pet').appendTo('.col-sm-6 col-md-4 col-lg-3');
   				$('<div>').attr('class','panel-heading').appendTo('.panel panel-default panel-pet');
   				$('<h3>').attr('class','panel-title').html(i.name).appendTo('.panel-heading');
   				
   				$('<div>').attr('class', 'panel-body').appendTo('.panel panel-default panel-pet');
   				$('<img>').attr('alt', '140x140').attr('data-src','holder.js/140x140').attr('class','img-rounded img-center').attr('src',i.picture).appendTo('.panel-body');
