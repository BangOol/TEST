...
<script>
	...
	function userDelete() {
		...
		$('body').on('click','#btnDelete',function(){
			...
			var userId = $(this).closet("tr").data("id");
			var result = confirm(userId +" 사용자를 정말로 삭제하시겠습니까?");
			if(! result) 
				return;

			$.ajax({
				url:conPath + '/userDelete/' + userId,
				type:'delete',
				dataType:'json'
			}).done(function(xhr) {
	...
	function userUpdate() {
		...
		$('#btnUpdate').on('click',function(){
			...
			$.ajax({ 
			    url: conPath + '/users', 
			    type: 'put', 
			    dataType: 'json', //응답 결과 JSON
			    contentType : 'application/json', // 요청 파라미터 JSON
			    data: JSON.stringify({id, name, password, role}),//$("#form1").serialize() // {id, name,password, role} 질의 문자열 p=v&..
			}).done( function(data) { 
			        userList();
	...
	function userList() {
		$.ajax({
			...
			for(item of datas){
				$('<tr>')
				.data("id", item.id) //밑의 append 대체 코드
